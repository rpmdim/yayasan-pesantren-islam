...
export default function NavbarRole() {
  ...
  const menu = user?.role === "Staff" ? staffMenu : memberMenu;
  const userName = user ? `${user.firstName} ${user.lastName}` : "";
  const roleLabel = user?.role ?? "";
  const dashboardHref = user?.role === "Staff" ? "/staff-dashboard" : "/dashboard";
          ...
          <div className="flex flex-row w-3/4 justify-end items-center gap-1">
            {menu.map((item) => (
              <Link key={item.href} href={item.href}>
                <button
                  className={`text-black font-medium text-[11px] min-h-[34px] px-3 flex justify-center items-center rounded-[8px] transition ${
                    pathname === item.href
                      ? "bg-[#CCA825]"
